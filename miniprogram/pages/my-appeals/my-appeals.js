    ...
    try {
      ...
      const formattedList = list.map(item => {
        ...
        return {
          ...
          displayTitle: (item.taskInfo && (item.taskInfo.title || item.taskInfo.description || item.taskInfo.locationName)) || item.reason || '申诉记录',
          displayTaskName: item.taskInfo ? typeMeta.name : '关联任务',
          displayTarget: item.respondentNickname || (item.targetUser && item.targetUser.nickname) || item.targetNickname || item.respondentUserNickname || '任务相关用户',
          displayTargetAvatar: item.respondentAvatar || (item.targetUser && item.targetUser.avatar) || item.targetAvatar || item.respondentUserAvatar || '',
          displayTargetRole: item.targetRoleLabel || (item.targetUser && item.targetUser.roleLabel) || '',
